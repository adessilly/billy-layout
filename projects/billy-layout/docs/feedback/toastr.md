...
- Tokens DS : `--billy-surface`, `--billy-surface-border`, `--billy-text-soft`, `--billy-text-muted`, `--billy-accent-strong`, `--billy-accent-soft`. Les teintes par type sont exposées via les variables locales `--t-accent` / `--t-icon-bg` (liseré gauche, pastille d'icône, barre de progression).
