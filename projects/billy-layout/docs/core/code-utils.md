...
Canonical `BE0123456749` · display `BE 0123.456.749` (prefix and dots grayed out). Abstract class, static methods only. **Only Belgium has an implemented check digit** (modulo 97); for 8 countries (`BE FR LU NL DE ES IT PT`) the body length is known and verified, and ~30 countries have a label. A conforming foreign number is `unverified`, never over-optimistically "valid".
...
const canon = VatUtils.sanitize(userInput);        // "be 0123.456.749" → "BE0123456749"
const mask = VatUtils.formatText(canon);           // "BE 0123.456.749" (shown in the input)
const info = VatUtils.describe(canon);             // { status: 'valid', message: 'Valid number', … }
const final = VatUtils.normalize('0123456749');    // "BE0123456749" on blur
```
