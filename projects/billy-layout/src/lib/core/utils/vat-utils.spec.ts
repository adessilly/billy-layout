...
describe('VatUtils', () => {
  ...
  describe('sanitize', () => {
    it('strips separators and uppercases', () => {
      expect(VatUtils.sanitize('be 0690.614.660')).toBe('BE0690614660');
    });
  ...
  describe('country', () => {
    it('returns the country code once both letters are typed', () => {
      expect(VatUtils.country('BE0690614660')).toBe('BE');
    ...
    it('returns null while the prefix is incomplete or numeric', () => {
      ...
      expect(VatUtils.country('0690614660')).toBeNull();
    });
  });

  describe('countryLabel', () => {
    it('uses the built-in English map by default', () => {
      expect(VatUtils.countryLabel('BE0690614660')).toBe('Belgium');
    ...
    it('translates through Intl.DisplayNames when a locale is given', () => {
      expect(VatUtils.countryLabel('BE0690614660', 'fr')).toBe('Belgique');
  ...
  describe('normalize', () => {
    it('prefixes a 10-digit number with BE', () => {
      expect(VatUtils.normalize('0690614660')).toBe('BE0690614660');
    });

    it('restores the leading zero of the old 9-digit Belgian format', () => {
      expect(VatUtils.normalize('690614660')).toBe('BE0690614660');
      expect(VatUtils.normalize('BE690614660')).toBe('BE0690614660');
    });
    ...
    it('leaves complete numbers untouched', () => {
      expect(VatUtils.normalize('BE0690614660')).toBe('BE0690614660');
  ...
  describe('format / formatText', () => {
    it('renders the canonical Belgian display', () => {
      expect(VatUtils.formatText('BE0690614660')).toBe('BE 0690.614.660');
    });

    it('marks the country prefix and separators as muted', () => {
      const segments = VatUtils.format('BE0690614660');
    ...
    it('groups digit-only input with the Belgian rule', () => {
      expect(VatUtils.formatText('0690614660')).toBe('0690.614.660');
    });
  ...
  describe('checksum', () => {
    it('validates a correct Belgian number (modulo 97)', () => {
      expect(VatUtils.checksum('BE0690614660')).toBe(true);
    });

    it('rejects a Belgian number with a wrong check digit', () => {
      expect(VatUtils.checksum('BE0690614661')).toBe(false);
    });
  ...
  describe('digits', () => {
    it('keeps only the digits', () => {
      expect(VatUtils.digits('BE 0690.614.660')).toBe('0690614660');
    });
  });
  ...
  describe('describe', () => {
    ...
    it('reports a valid Belgian number', () => {
      const info = VatUtils.describe('BE0690614660');
    ...
    it('reports an invalid Belgian check digit', () => {
      expect(VatUtils.describe('BE0690614661').status).toBe('invalid');
    });
    ...
    it('reports partial input while typing', () => {
      expect(VatUtils.describe('BE06906').status).toBe('partial');
